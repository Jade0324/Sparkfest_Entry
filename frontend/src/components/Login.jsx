...
export default function Login({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      await fetch('http://[https://sparkfest-entry-1.onrender.com](https://sparkfest-entry-1.onrender.com)/api/auth/login', {
        method: 'POST',
    ...
    <div className="flex flex-col h-full bg-[#F8F9FC] overflow-hidden">
      ...
      <div className="flex-1 px-6 pt-6 pb-8 overflow-y-auto">
        ...
        <div className="flex items-center justify-center gap-1 mt-6">
          ...
          <span className="text-xs font-extrabold text-[#1A2C5B]">Talino</span>
          <span className="text-xs font-extrabold text-[#F8A03E]">Tini</span>
          <span className="text-xs font-extrabold text-[#F495A5]">g</span>
        </div>
      </div>
    </div>
  );
