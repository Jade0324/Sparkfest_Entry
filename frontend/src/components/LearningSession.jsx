...
// ═══════════════════════════════════════════════════════════
// MASCOT — swap SVG bodies later without touching logic
// ═══════════════════════════════════════════════════════════
function Mascot({ state = 'idle' }) {
  const states = {
    ...
    listening: {
      body: '🦜',
      bubble: 'I\'m listening… speak up! 🎙️',
    },
    celebrating: {
      body: '🦜',
      bubble: 'Amazing job! Let\'s unlock the next part of our adventure! 🌟',
    },
    encouraging: {
      body: '🦜',
      bubble: 'Almost there! Let\'s give it another try together! 💪',
    },
  };
    ...
    <div className="flex flex-col items-center gap-1 select-none">
      ...
      {current.bubble && (
        <div className="relative max-w-[220px]">
          ...
          <div className={`rounded-2xl px-3 py-2 text-center text-xs font-semibold leading-snug shadow-sm ${
...
// ═══════════════════════════════════════════════════════════
// CONFETTI — pure CSS, no library needed
// ═══════════════════════════════════════════════════════════
function Confetti() {
  const pieces = ['🌟', '⭐', '✨', '🎉', '🎊', '💫', '🌈'];
  return (
...
// ═══════════════════════════════════════════════════════════
// LEVEL CONFIG — derived from an exercise object
// ═══════════════════════════════════════════════════════════
...
function computeAccuracy(spoken, target) {
  ...
  // For phrases/sentences, score the whole string not just one word
  ...
  // Also try word-by-word best match for single word targets
...
// ═══════════════════════════════════════════════════════════
// EMOJI fallback when no image URL
// ═══════════════════════════════════════════════════════════
const EMOJI_MAP = {
...
export default function LearningSession({ sessionId, onComplete, onBack }) {
  ...
  // Exercise navigation
  const [exIndex,       setExIndex]       = useState(0);   // which exercise card
  const [currentLevel,  setCurrentLevel]  = useState(0);   // 0=L1, 1=L2, 2=L3
  ...
  // Result state: null | 'correct' | 'incorrect'
  ...
  // Per-exercise level results: { [exIndex]: { [level]: { passed, accuracy } } }
  const [levelResults,  setLevelResults]  = useState({});

  // Local attempts (for summary screen)
    ...
    try {
      const res = await fetch(`${API}/sessions/${s.id}/attempts`, {
        ...
        body: JSON.stringify({
          ...
          // level info sent as part of audioUrl metadata for now
          // TODO: add `level` field to AttemptRequest when backend supports it
        }),
      });
      ...
      setLocalAttempts(prev => [...prev, enriched]);

      // Record level result
      setLevelResults(prev => ({
        ...prev,
        [idx]: { ...(prev[idx] || {}), [lvl]: { passed, accuracy } },
      }));
  ...
  const handleTryAgain = () => {
    ...
    setLastSpoken('');
    setInterim('');
    // Play the target word via TTS as acoustic model hint
    const s   = sessionRef.current;
    const idx = exIndexRef.current;
    const lvl = levelRef.current;
    if (!s) return;
    const exercise = s.exercises[idx]?.exercise;
    if (!exercise) return;
    const target = getLevels(exercise)[lvl].target;
    speakWord(target);
  };
  ...
  const handleNext = async () => {
    ...
    if (lvl < 2) {
      // Advance to next level within same exercise
      setCurrentLevel(lvl + 1);
    } else {
      // All 3 levels done — advance to next exercise or complete
      const nextIdx = idx + 1;
      ...
      <div className="flex flex-col h-full bg-[#F0F4FF] overflow-y-auto p-5">
        ...
        <div className="bg-white rounded-[32px] shadow-lg p-6 w-full">
          ...
          <div className="flex justify-center gap-2 mb-5">
            {[1,2,3].map(i => (
              <span key={i}
                className={`text-4xl transition-all ${i <= stars ? '' : 'grayscale opacity-25'}`}>
                ⭐
              </span>
            ))}
          </div>
                ...
                <div key={ei}
                  ...
                  <div className="flex gap-2">
                    {levels.map((lv, li) => {
                      ...
                      return (
                        <div key={li}
                          className={`flex-1 rounded-xl p-2 text-center text-[10px] font-bold ${
  ...
  // Overall progress: each exercise has 3 levels
  const totalSteps    = exercises.length * totalLevels;
  const doneSteps     = exIndex * totalLevels + currentLevel;
  const progressPct   = totalSteps > 0
    ? Math.round((doneSteps / totalSteps) * 100) : 0;
  ...
  return (
    <div className="flex flex-col h-full bg-[#F0F4FF] overflow-hidden relative">
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-4 flex flex-col gap-4">
        ...
        {/* Prompt card */}
        <div className={`rounded-3xl p-4 text-center border-2 transition-all duration-500 ${
          levelColor.bg} ${levelColor.ring} ring-2`}>
        ...
        <div className="relative bg-white rounded-[28px] shadow-md overflow-hidden
          ...
          {activeLv?.image
            ? <img
              ...
              />
            : <span className="text-[100px] leading-none py-3">
          ...
          <button
            onClick={() => speakWord(activeLv?.target)}
        ...
        <div className="text-center">
          ...
          <p className="text-3xl font-extrabold text-[#1A2C5B] tracking-tight
            leading-tight">
            {activeLv?.target}
          </p>
        </div>
          ...
          <div className="flex flex-col items-center gap-2 pb-2">
            <div className="relative">
              ...
              <button
                ...
                disabled={submitting}
                className={`relative w-20 h-20 rounded-full flex items-center
                  justify-center text-white shadow-lg transition-all active:scale-95
                  disabled:opacity-60 ${
                  listening
          ...
          <div className="flex gap-3 pb-2">
            {resultState === 'incorrect' && (
              <button
                ...
                className="flex-1 py-4 border-2 border-gray-200 bg-white
                  text-gray-600 rounded-2xl font-bold active:scale-95 transition-transform">
                Try Again
              </button>
            )}
            <button
