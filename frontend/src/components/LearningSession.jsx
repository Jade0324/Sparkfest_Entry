...
export default function LearningSession({ sessionId, onComplete, onBack }) {
  ...
  // KEY FIX: ref that stays in sync with levelResults so handleNext
  // can read the latest data without stale closure issues
  ...
  const handleSpeechResult = async (spoken) => {
    ...
    try {
      ...
      // ── KEY FIX: update both state AND ref so handleNext sees latest data ──
      setLevelResults(prev => {
        ...
        levelResultsRef.current = next;   // keep ref in sync
        return next;
      });
  ...
  const handleNext = async () => {
    ...
    if (lvl < 2) {
      ...
      if (nextIdx >= exercises.length) {
        // ── Session complete — save levelHistory to localStorage ──
        try {
          const res  = await fetch(`${API}/sessions/${sessionId}/complete`, { method: 'PATCH' });
          const data = res.ok ? await res.json() : { totalScore: null };

          // Build levelHistory: exerciseId → { targetWord, level texts, per-level results }
        ...
        <div className="relative bg-white rounded-[28px] shadow-md overflow-hidden
          flex items-center justify-center" style={{ minHeight: 160 }}>
            ...
            ? <img
                ...
                onError={(e) => {
                    // If image URL is broken, hide img and show emoji fallback
        ...
        {!resultState ? (
          <div className="flex flex-col items-center gap-2 pb-2">
            <div className="relative">
              ...
              <button onClick={handleMicClick} disabled={submitting}
                className={`relative w-20 h-20 rounded-full flex items-center justify-center
                  text-white shadow-lg transition-all active:scale-95 disabled:opacity-60 ${
                  listening
